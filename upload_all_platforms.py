"""
Lingexa Phrasal - Unified Social Media Upload Script
"""
...
def get_latest_reel():
    ...
    return {"video_path": str(latest), "metadata": metadata, "words": words_data, "all_words": all_words, "word": all_words[0] if all_words else "Phrasal Verb"}

def generate_caption(reel_data, platform="facebook"):
    words = reel_data.get("words", [])
    if not words:
        return f"Master phrasal verbs with {CHANNEL_NAME}! #LingexaPhrasal #English"
    if platform == "facebook":
        lines = [f"📚 Master 5 English Phrasal Verbs with {CHANNEL_NAME}!", f"", f"Today's Phrasal Verbs:", f""]
        for i, w in enumerate(words, 1):
            ...
            lines.append(f"")
        lines.extend([f"💡 Save and practice these phrasal verbs!", f"🔔 Follow {CHANNEL_NAME} for daily lessons!", f"", f"#LingexaPhrasal #PhrasalVerbs #EnglishGrammar #LearnEnglish #EnglishVocabulary #ESL #EnglishTips #Grammar"])
    else:
        lines = [f"📚 5 Phrasal Verbs to Learn Today!", f""]
        for i, w in enumerate(words[:3], 1):
            word = w.get("word", "")
            definition = w.get("definition", "")
            lines.append(f"{i}. {word} - {definition}")
        lines.extend([f"", f"#LingexaPhrasal #PhrasalVerbs #English"])
    return "\n".join(lines)
        ...
        if upload_func:
            try:
                if platform_name == "facebook":
                    upload_result = upload_func(video_path=video_path, description=caption, title=f"Phrasal Verb: {word}")
...
def main():
    ...
    if not reel:
        print("No reel found! Run lingexa_phrasal_bot.py first.")
        sys.exit(1)
