"""
Lingexa Phrasal - English Phrasal Verbs Mastery
Master English phrasal verbs with examples and explanations
"""
...
WORDS_PER_VIDEO = 3
WORD_HISTORY_FILE = HISTORY_DIR / "all_generated_phrasal.json"
FONTS_DIR = Path(__file__).parent / "fonts"

WORD_LEVELS = ["Beginner", "Intermediate", "Advanced", "Business", "Daily Life", "Academic", "Slang"]

def load_word_history():
    if WORD_HISTORY_FILE.exists():
        with open(WORD_HISTORY_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"phrasals": [], "last_updated": None}
...
def is_word_used(word):
    ...
    return word.lower().strip() in [w.lower().strip() for w in history.get("phrasals", [])]

def add_words_to_history(words):
    history = load_word_history()
    existing = [w.lower().strip() for w in history.get("phrasals", [])]
    for w in words:
        word_lower = w.lower().strip()
        if word_lower not in existing:
            history["phrasals"].append(word_lower)
...
def generate_word_data(num_words: int = WORDS_PER_VIDEO) -> list:
    ...
    categories = [
        ...
        "business and work phrasal verbs",
        "travel and transport phrasal verbs",
        "emotions and relationships phrasal verbs",
        "academic and study phrasal verbs",
        ...
        "sports and competition phrasal verbs",
        "weather and nature phrasal verbs",
        "housework phrasal verbs",
    ]
    collected = []
    collected_lower = set()
...
STRICT RULES:
- NEVER repeat any of these: {used_list}
- Each phrasal verb must include the verb + particle (e.g. "give up", "look after")
- Return ONLY a valid JSON array
- Every phrasal verb must be different from all others in your response
- Choose USEFUL and COMMON phrasal verbs that learners actually need

Format for each phrasal verb:
{{"word": "give up", "particle": "up", "part_of_speech": "phrasal verb", "definition": "to stop trying or doing something", "example": "She gave up smoking last year.", "synonyms": ["quit", "stop", "surrender"], "fun_fact": "Separable: You can say 'give it up' or 'give up smoking'"}}

KEEP EVERY FIELD SHORT - definition max 8 words, example max 8 words, fun_fact max 10 words.

Return ONLY the JSON array. Nothing else."""
            payload = {"model": AI_MODEL, "messages": [{"role": "system", "content": "You are a phrasal verb expert. Return ONLY valid JSON arrays."}, {"role": "user", "content": prompt}], "temperature": 1.5}
            ...
            print(f"[api] AI returned {len(words)} phrasals: {[w.get('word', '') for w in words]}")
            fresh_this_round = []
            skipped = []
            for w in words:
                ...
                if not word:
                    ...
                    continue
                if len(word.split()) < 2:
                    skipped.append(f"{word}(not a phrasal verb)")
                    continue
            ...
            if len(collected) >= num_words:
                ...
                print(f"[api] SUCCESS: Got {len(collected)} fresh phrasals on attempt {attempt + 1}")
...
def create_background():
    ...
    for y in range(VIDEO_HEIGHT):
        ...
        if ratio < 0.5:
            ...
            r = int(235 + (220 - 235) * ((ratio - 0.5) * 2))
            g = int(248 + (240 - 248) * ((ratio - 0.5) * 2))
            b = int(255 + (250 - 255) * ((ratio - 0.5) * 2))
...
def generate_word_image(word_data: dict, bg_image, output_path: str):
    ...
    font_syn_label = load_font(fonts_bold, 42)
    font_syn = load_font(fonts_regular, 48)
    ...
    particle = word_data.get("particle", "")
    definition = word_data["definition"]
    example = word_data["example"]
    synonyms = word_data.get("synonyms", [])
    fun_fact = word_data.get("fun_fact", "")
    ...
    draw.rectangle([(0, 0), (VIDEO_WIDTH, 90)], fill=(0, 100, 148))
    ...
    draw.text((CENTER_X, y_cursor), word, fill=(0, 60, 100), font=word_font, anchor="mm", stroke_width=max(1, word_font_size // 40), stroke_fill=(200, 230, 250))
    y_cursor += word_h + 50
    ...
    if level:
        ...
        draw.rounded_rectangle(
            ...
            radius=10, fill=(0, 100, 148)
        )
        draw.text((CENTER_X, y_cursor + level_h // 2 + 8), level_text, fill=(255, 255, 255), font=font_level, anchor="mm")
        y_cursor += level_h + 55

    def_label = "MEANING"
    draw.text((MARGIN_X, y_cursor), def_label, fill=(0, 100, 148), font=font_def_label, anchor="lm")
    y_cursor += 60
    ...
    def_box = Image.new('RGBA', (CONTENT_WIDTH, def_box_h), (0, 100, 148, 255))
    def_draw = ImageDraw.Draw(def_box)
    def_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, def_box_h)], radius=18, fill=(0, 100, 148, 255))
    ...
    ex_label = "EXAMPLE"
    draw.text((MARGIN_X, y_cursor), ex_label, fill=(0, 100, 148), font=font_ex_label, anchor="lm")
    y_cursor += 60
    ...
    ex_box = Image.new('RGBA', (CONTENT_WIDTH, ex_box_h), (180, 215, 240, 220))
    ex_draw = ImageDraw.Draw(ex_box)
    ex_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, ex_box_h)], radius=15, fill=(180, 215, 240, 220))
    for i, line in enumerate(ex_lines):
        line_y = ex_padding + (i * ex_line_spacing) + (ex_line_height // 2)
        ex_draw.text((CONTENT_WIDTH // 2, line_y), line, fill=(0, 50, 80), font=font_ex, anchor="mm")
    img.paste(ex_box, (MARGIN_X, y_cursor), ex_box)
    y_cursor += ex_box_h + 65

    if synonyms:
        syn_label = "SIMILAR"
        draw.text((MARGIN_X, y_cursor), syn_label, fill=(0, 100, 148), font=font_syn_label, anchor="lm")
        y_cursor += 60
        syn_text = ", ".join(synonyms[:4])
        syn_lines = wrap_text(draw, syn_text, font_syn, CONTENT_WIDTH - 70)
        while len(syn_lines) > 2 and font_syn.size > 36:
            font_syn = load_font(fonts_regular, font_syn.size - 4)
            syn_lines = wrap_text(draw, syn_text, font_syn, CONTENT_WIDTH - 70)
        for i, line in enumerate(syn_lines):
            ...
            draw.text((CENTER_X, line_y), line, fill=(0, 80, 120), font=font_syn, anchor="mm")
        y_cursor += len(syn_lines) * syn_line_height + 60

    if fun_fact:
        ff_label = "GRAMMAR TIP"
        draw.text((MARGIN_X, y_cursor), ff_label, fill=(0, 100, 148), font=font_ff_label, anchor="lm")
        y_cursor += 60
        ...
        ff_box = Image.new('RGBA', (CONTENT_WIDTH, ff_box_h), (255, 220, 160, 200))
        ff_draw = ImageDraw.Draw(ff_box)
        ff_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, ff_box_h)], radius=14, fill=(255, 220, 160, 200))
    ...
    draw.rectangle([(0, VIDEO_HEIGHT - 65), (VIDEO_WIDTH, VIDEO_HEIGHT)], fill=(0, 100, 148))
    draw.text((CENTER_X, VIDEO_HEIGHT - 32), f"Phrasal Verbs Daily  |  {CHANNEL_NAME}", fill=(200, 230, 250), font=font_footer, anchor="mm")
...
def generate_reel():
    print(f"\n{'='*80}\n  {CHANNEL_NAME.upper()} - ENGLISH PHRASAL VERBS\n{'='*80}\n")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    reel_dir = VIDEO_DIR / f"phrasal_{timestamp}"
    reel_dir.mkdir(exist_ok=True)
    print("[1/5] Generating 5 phrasal verbs...")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\n  Master phrasal verbs | BOLD visuals | Grammar tips")
    print("="*80)
    generate_reel()
